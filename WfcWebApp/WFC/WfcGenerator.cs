...
public class WfcGenerator
{
	...
	private int backpropEntropyThreshold = 10; //maximum number of unique patterns we're willing to analyze during backprop
	...
	private HashSet<Vector2I> collapsedPositionTracker = new();
	...
	private void SingleGenerationCycleBounded(Vector2I collapse_pos, float radius=10) {
		if (wfcWave.IsUnobserved(collapse_pos)) {
			...
			wfcWave.GetOrCreatePatternSet(collapse_pos, out HashSet<Pattern> waveSet);
	...
	private void SingleBackpropStep(Vector2I backprop_pos, Circle boundary) {
		...
		foreach (Vector2I neighbor_pos in Vector2I.Neighbors(backprop_pos)) {
			...
			HashSet<Pattern> unionPatternSet = new();
	...
	private void PerformBackpropBounded(Vector2I collapse_pos, float radius) {
		...
		Circle boundary = new Circle(collapse_pos, radius);
