...
public class WfcWave
{
    ...
    public IEnumerable<int> EnumeratePatternSet(Vector2I pos) {
        if (TryAccessWave(pos, out SparsePatternSet patternSet)) {
            foreach (int index in patternSet) {
                yield return index;
            }
        }
    }
    ...
    public bool GetOrCreatePatternSet(Vector2I pos, out SparsePatternSet patternSet) {
    ...
    private bool TryAccessWave(Vector2I pos, out SparsePatternSet patternSet) {
    ...
    private RandomUtils.WeightedKeyCounter<Vector2I> keyCounter = new();
    ...
    public bool GetLeastEntropyPosition(out Vector2I leastEntropyPos, IEnumerable<Vector2I>? positionFilter = null, bool includeCollapsed = false) {
        ...
        if (positionFilter == null) {
            // default to searching the entire wave
            if (boundaryCorner.HasValue) {
                positionFilter = MathUtils.GridEnumerator(boundaryShape.X, boundaryShape.Y);
            } else {
                positionFilter = waveDict.Keys;
            }
        }
        keyCounter.Clear();
        ...
        foreach (Vector2I pos in positionFilter) {
            int entropy = GetEntropy(pos);           //TODO: entropy isnt just about whether it fits, but about it's weight too
    ...
    public bool IsUnobserved(Vector2I pos) {
        if (TryAccessWave(pos, out SparsePatternSet waveSet)) {
